      ...
      windowListener: window.addEventListener('keydown', event => {
        ...
        if ([8, 46].indexOf(event.keyCode) !== -1 && selectedItems.length && deleteItems) {
          selectedItems.forEach(element => {
            // The option to delete text nodes with the "delete" key is disabled to allow deletion of text within editable nodes.
            if (element.nodeType !== 'textnode') element.remove();
          });
